...
# Review smoke/formal cache template consistency

Review independently; you did not implement this patch. Your earlier review300c4873 is archived. Use this new task and workspace only. Model gpt-5.6-terra/max.

Create RMBench worktree at 9d8f47887a50ea691e5624de139f10bfcfb54412 and robot-bridge worktree at f9626636c4776d8eb15f9c556775cb2d12c000e5 with mam workspace add, then read their AGENTS.md. No OpenPI environment or GPU run is necessary for this narrowly scoped CPU review.

Source task e6908de7-4b02-465a-987b-a19eba7a315a: f508749 passed a C smoke but formal was rejected before reset because literal per-run WARP_CACHE_PATH differed. Patch9d8f478 changes only the cache command to runner's existing {result_run} template. Prior report accidentally gives a different full hash starting9d8f478; exact source of truth for this review is the full commit above.

Verify through actual frozen BenchmarkRunner code, not merely string checks: same checkpoint/eval-seed smoke+formal templates satisfy strict compatibility; actual child cache paths differ by result_run and by robot/policy; distinct concurrent runs/hosts remain isolated with unique run names; templating does not change result location, commands, seeds, memory, horizon or existing gate checks. Old smoke cannot authorize changed launch; rerun fresh matching smoke2 then formal100. Reproduce old failure and patched success with a bounded CPU fixture; run appropriate existing runner tests. No remote/GPU/production changes.

Publish a concise report with exact reviewed commits, reproduction and tests, PASS or concrete blockers. Clean own temporary files; leave clean worktrees for Manager archive. End turn after delivery. Manager handles deployment.
