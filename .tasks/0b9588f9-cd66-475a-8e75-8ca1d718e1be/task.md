...
## 评测口径更新（用户明确）
默认每个模型3个eval seed，各100 episodes，共300；先按300汇总再比较training seed差异。现有单eval0对比只作中间证据，不能提前认定训练不稳定。请核查当前完整300是否足够比较，不能把不完整eval混入。优先覆盖九任务，训练重复由Manager按汇总证据决定。补充事实：每个任务现有同骨干历史/显式状态基线与Mem-0/DM05是否真正有可加载可比资产；pose/连续字段是否实现以及具体缺项。不要新开跑。
