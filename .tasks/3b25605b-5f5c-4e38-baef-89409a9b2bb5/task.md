...
用户最新追加：管理库 .worklogs 已删除，现行文档统一使用.tasks，并明确所有agent/管理CLI在本机运行。源码验收仍固定48d188b；文档额外读取 main 上最新AGENTS、README和接口说明，不必为纯文档变更重新安装环境或重跑已通过代码测试。报告注明实际读取的文档commit及最新本任务发布版本。
...
3. 只启动自己的短时CPU测试进程，本机和wuwen-1各一个，实际通过CLI job add登记到本任务，验证running→stopped→archived；真实App Server查询自己的agent状态。测试停止/清理后记录结果并归档job。不要用新server或伪造agent状态冒充现场验证。
   用户已明确运行边界：所有agent、代码修改和管理CLI均在本机，只有GPU作业通过SSH在wuwen-1运行。验收重点为本机管理CLI正确查询本机与显式host=wuwen-1的作业。跨主机执行管理CLI不作为支持目标；此前已做的localhost换主机查询可作为限制记录，不为这一不支持的用法扩大实现。管理库改名/MAM安装方式仍在与用户讨论，本次不改名或安装。
