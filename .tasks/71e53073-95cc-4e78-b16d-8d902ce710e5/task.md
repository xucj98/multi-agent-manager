...
## 指南一致性核对
用户询问远程eval指南是否存在及是否被要求阅读。已确认C根README《集群C State-VLA评测操作手册》存在，e690 task明确要求读取。请在审计中核对该指南与当前脚本/资源/流程一致性，包含其可发现性、旧openpi-client说明、每次创建都测耗时空间的必要性、smoke数量措辞、结果“新exp-group归档”是否偏离任务组内一致布局，以及新版job归档语义。只报告有依据的不一致，不自动改文档。
