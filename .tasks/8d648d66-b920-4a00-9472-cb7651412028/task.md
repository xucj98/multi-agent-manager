...
## Follow-up review: installer and setup races
Re-review latest main after integrated 675a533 (main60cfd89) and 679c859 installer follow-up. Check your four accepted findings against actual changes; verify deterministic original-style restart, exit-status propagation, pre-compat trace boundary and stale row filtering. Review final README/AGENTS/design/install docs for concise, clear, consistent user operations. Do not restart production app-server. Native Manager send_input smoke on60cfd89 FAILED (wait returned job_stopped after180s instead of message); runtime owner is investigating, so do not certify native wake or block this focused installer review on its pending fix. Report concrete blockers with smallest correction; include exact reviewed commits. Existing worktree and task remain in use.
