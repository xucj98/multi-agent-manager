...
checkpoint：`/mnt/public/xcj/Projects/openpi/checkpoints/pi05_rmbench_put_back_block_full_t_plus_1/memory20k_e7e5ac54_put_back_full_t_plus_1_s2/20000`。
本机固定解释器显式CUDA_VISIBLE_DEVICES为空/JAX_PLATFORMS=cpu，实际读回51叶/3353433872元素，全BF16、全有限、全部参数路径与注册config shape一致，CPU核验exit0。父目录仅20000，有原子commit，仅params/assets/metadata，无optimizer；metadata JSON/JSONL/YAML解析通过，d10/clean/实际command/config协议一致，demo_clean_state来源和norm与验收资产吻合。200条标量均有限，Step20000：grad_norm=0.0325, loss=0.0006, param_norm=1804.4269，未见训练报错。

逐参数shape及完整CPU审计：`/mnt/public/xcj/Projects/workspace/e7e5ac54-2f5c-4f46-9210-6c2a51f7f4fe/closure/memory20k_e7e5ac54_put_back_full_t_plus_1_s2.json`。
对应training job按结果归档；累计13项完成/1项未完成。本任务不在wuwen-1接续任何GPU工作；06:05记录的外部不可见占用边界继续保留，本轮未重新探测远端；本机释放卡交Manager安排eval；完整policy GPU恢复/wire/评测未由本任务擅自启动。继续MAM事件等待，剩余GPU0于12:32巡检。

## 9月11日11:32 本机最后两路小时巡检

实时job status均running，无error。日志持续前进，已落盘标量全部有限，无可见训练报错；其余12项已收尾归档，未重复checkpoint读取或远端探测。

| 本机GPU | 模型/seed | 最新真实标量 | 剩余ETA | 预计完成更新 |
| --- | --- | --- | --- | --- |
| 0 | put-back full t+30 / 2 | Step 18900: grad_norm=0.0300, loss=0.0006, param_norm=1804.2607 | 1:04:09 | 约12:36 |
| 1 | put-back full t+1 / 2 | Step 19100: grad_norm=0.0301, loss=0.0006, param_norm=1804.4077 | 54:05 | 约12:26 |

两路各189/191条标量为区间均值，ETA不含最终保存。冻结运行树d10完整SHA正确，git status为空，源码/参数未变。

资源快照：0, 73417 MiB, 7622 MiB, 100 %；1, 73405 MiB, 7633 MiB, 100 %；RAM available 886Gi。

继续active turn与mam wait等待完成事件，下一小时12:32；新完成checkpoint按既有收尾要求验收。wuwen-1保持停用，GPU恢复/评测由Manager排期。
...
| 本机GPU | 模型/seed | 最新真实标量更新 | loss / grad_norm / param_norm | 剩余ETA / 预计完成更新 |
| --- | --- | --- | --- | --- |
| 0 | put-back t+30 / 2 | Step17500 | 0.0006 / 0.0300 / 1804.2233 | 2:30:04 / 约12:36 |
| 1 | put-back t+1 / 2 | Step17700 | 0.0008 / 0.0341 / 1804.3680 | 2:17:03 / 约12:23 |

各175/177条已落盘标量全部有限（区间均值）；无可见Traceback/CUDA/RESOURCE_EXHAUSTED/OOM。稳定约3.6秒/update，ETA不含最终保存。GPU0用73427MiB、空闲7611MiB、100%；GPU1用73405MiB、空闲7633MiB、100%；RAM可用876GiB。冻结源码HEAD正确且git status为空。

其余12项已验收并归档，不重复CPU读取。wuwen-1不再启用；此前本任务全部释放与外部不可见占用的边界保持不变。继续复用既有mam wait等待结束事件，按任务核验新20000 checkpoint/metadata、CPU参数恢复及进程/子进程/显存释放后归档；GPU策略恢复/评测仍由Manager排期。下一小时检查10:32。
