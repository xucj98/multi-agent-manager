...
`b049801` 与最终 `9fe95e0a088a48cfa6ee7bd5b0edcfa1113702c7`。
...
- base/HEAD：`9fe95e0a088a48cfa6ee7bd5b0edcfa1113702c7`
...
执行 `cleanup.sh` 后，git ignored 状态仅为 `.cache`、`.venv/`、`outputs`。直接调用 MAM 的只读 `dirty()` 归档预检返回恰这三项，均属于 MAM 允许移除/保留的类别；canonical `outputs/TASK-ID` 的 smoke 成果仍存在。未执行实际 archive，以保留 review worktree 供 Manager 验收。repair 后 uv store 仅再增 19,968 B。

## 运行验证

- `git diff --check`、所有 worktree environment shell 脚本 `bash -n`：PASS。
