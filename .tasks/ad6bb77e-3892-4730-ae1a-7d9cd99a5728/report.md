当前结果：完成本次wash两路小时巡检，实际快照2026-09-10 23:32:31 CST。两路实时running、更新持续增加，全部可见标量有限，无新增异常。下一检查2026-09-11 00:32 CST，由Manager按小时或作业结束唤醒；结束本次巡检。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 2 / full | 12300 | 12200 | 0.0072 | 0.0480 | 1805.9047 | 3.6905 | 2026-09-11 07:27左右 |
| 3 / serial | 12300 | 12200 | 0.0256 | 0.7466 | 1806.0573 | 3.6896 | 2026-09-11 07:27左右 |

- 两路各122组已落盘loss/grad_norm/param_norm全部有限，较本owner前次20:58快照各新增25组；未冒称期间另做巡检。标量为每100更新区间均值，不代表逐update原始标量。
- 万步后progress取整到百位，12300为约数，最近精确标量step均12200。吞吐取精确scalar step11200→12200后首条progress时间差，避免百位取整扭曲一小时速率；两路约975–976 updates每小时。ETA为估算，含日志时间定位误差且最终保存另需耗时。
- 当前安装的job status于23:31:51实时确认两路running、未返回error，身份由MAM内部核对。原PID、cwd及python -u -B/batch32/seed0/20000更新/仅最终保存命令匹配。
- 全日志无所检错误或非有限标量，无新增警告。日志距采样full4.4秒、serial9.5秒。
- GPU2/3占用73406/73408MiB、空闲7633/7631MiB，利用率均100%，温度68/56°C；只查询本任务GPU2/3。
- 本次按现有安装操作，报告不再含task_revision首行；未提前采用仍在实施的项目配置迁移。
- 调度遵守最新要求：wuwen-1结束保存释放后整机空闲；本机8卡可按原授权接续，启动前核对可用性。本次未启动GPU程序或重复smoke。
- 收尾待20k完成后核对退出状态、唯一20000及完整参数/metadata/assets、checkpoint-only恢复、本任务登记进程及子进程退出和显存释放。仅清理确认属于本任务的残留并记录，不处理不明进程。
...
本次快照：/mnt/public/xcj/Projects/workspace/ad6bb77e-3892-4730-ae1a-7d9cd99a5728/wash_gpu_logs/formal_hourly_20260910_2332.json
...
剩余：两路20k继续运行；下一巡检2026-09-11 00:32 CST。完成后按task验收唯一20000、完整shape/BF16/model-only、metadata/assets和checkpoint-only恢复，交Manager安排5ep offline，再处理产物与MAM job收尾。
