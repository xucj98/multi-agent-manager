...
当前结果：完成15:57 wash例行巡检，实际快照2026-09-10 15:58:57 CST。两路实时running、更新持续增加，全部可见标量有限，无新增异常；下一检查2026-09-10 16:57 CST。本次仅只读巡检并发布报告，结束本次巡检。
...
| GPU/配置 | progress约数 | 较14:58新增约数 | 可见标量step | loss | grad_norm | param_norm | 秒/更新 | ETA（北京时间） |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 2 / full | 4900 | 980 | 4900 | 0.0131 | 0.0631 | 1803.9165 | 3.7287 | 2026-09-11 07:37左右 |
| 3 / serial | 4910 | 990 | 4900 | 0.0451 | 1.6460 | 1804.0315 | 3.6992 | 2026-09-11 07:29左右 |

- 两路各49组已落盘loss/grad_norm/param_norm全部有限，较上次各新增10组。标量为每100更新区间均值，不代表逐update原始标量。
- progress按kit取整；最近精确标量step均4900。吞吐按上次与本次日志时间差及约数增量计算，约965/973 updates每小时。ETA为估算，受取整、吞吐及最终保存耗时影响。
- 当前旧版JSON mam job list --task在15:58:04实时核验两job running；未使用尚未安装的新wait/列表接口，也未将task status缓存当实时证据。两原PID/start_ticks、cwd及python -u -B/batch32/seed0/20000更新/仅最终保存命令均匹配。
- 全日志无所检错误或非有限标量，无新增警告。日志距采样full8.3秒、serial2.1秒。
- GPU2/3占用73406/73408MiB、空闲7633/7631MiB，利用率均100%，温度70/57°C；只查询本任务GPU2/3。
...
本次快照：/mnt/public/xcj/Projects/workspace/ad6bb77e-3892-4730-ae1a-7d9cd99a5728/wash_gpu_logs/formal_hourly_20260910_1557.json
...
剩余：两路20k继续运行；下一巡检2026-09-10 16:57 CST。完成后按task验收唯一20000、完整shape/BF16/model-only、metadata/assets和checkpoint-only恢复，交Manager安排5ep offline，再处理产物与MAM job收尾。
