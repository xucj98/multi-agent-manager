当前结果：完成00:32 wash两路小时巡检，实际快照2026-09-11 00:33:12 CST。两路最新日志持续推进，全部可见标量有限，无新增异常。下一检查2026-09-11 01:32 CST，由Manager唤醒；结束本次巡检。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 2 / full | 13300 | 13200 | 0.0073 | 0.0503 | 1806.0789 | 3.6888 | 2026-09-11 07:27左右 |
| 3 / serial | 13300 | 13200 | 0.0274 | 0.6593 | 1806.2277 | 3.6893 | 2026-09-11 07:27左右 |

- 两路各132组已落盘loss/grad_norm/param_norm全部有限，较23:32快照各新增10组；progress约各增加1000。标量为每100更新区间均值，不代表逐update原始标量。
- 万步后progress取整到百位，13300为约数，最近精确标量step均13200。吞吐取精确scalar step12200→13200后首条progress时间差，已处理跨午夜日期；两路约976 updates每小时。ETA为估算，最终保存另需耗时。
- Manager已确认16个job均running，本轮未重复MAM作业探测。两路原PID存在、cwd和python -u -B/batch32/seed0/20000更新/仅最终保存命令匹配，日志距采样full0.6秒、serial5.1秒。
- 全日志无所检错误或非有限标量，无新增警告。GPU2/3占用73406/73408MiB、空闲7633/7631MiB，利用率均100%，温度69/57°C；只查询本任务GPU2/3。
- 已读取迁移后的最新task，直接mam操作，不用--root；报告无task_revision，按项目配置发布到project/state-vla。
- 调度保持：wuwen-1结束保存释放后整机空闲；本机8卡按原授权使用且启动前核对可用性。本次未启动GPU程序或重复smoke。
...
本次快照：/mnt/public/xcj/Projects/workspace/ad6bb77e-3892-4730-ae1a-7d9cd99a5728/wash_gpu_logs/formal_hourly_20260911_0032.json
...
剩余：两路20k继续运行；下一巡检2026-09-11 01:32 CST。完成后按task验收唯一20000、完整shape/BF16/model-only、metadata/assets和checkpoint-only恢复，交Manager安排5ep offline，再处理产物与MAM job收尾。
