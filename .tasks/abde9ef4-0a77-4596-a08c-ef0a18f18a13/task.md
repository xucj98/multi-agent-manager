...
使用 gpt-5.6-terra / max。先读 MAM AGENTS/README/.local，mam task show 本任务及源任务8968b7f5-74f7-44db-ad0b-058d3fd556ca的最新要求。Manager负责科学裁决：同一加载实例、同输入/key的logging off/on invariance，与跨进程重放saved output分开；J前者成立，后者在actions正确float32边界后仍不等。不能降低数值标准或掩盖failed旧记录。本轮只核查下列新任务私有工具，不重新审计冻结三库，不跑GPU/model/reset，不改作者工具。

候选目录 /mnt/public/xcj/Projects/workspace/8968b7f5-74f7-44db-ad0b-058d3fd556ca/c2-query-diagnostic-invariance-replay-s-continuation 。prepared receipt ca94e5fd0b9f0ea5dcf507474c49d6c73462abe097b9d025a897aab18c2f9a96；工具manifest 46cb62aad57e883d849b37d5871547656fb0469c6ec8b8e3d87df5a236e86443；pair script acabef4d10f45f4e5ade67b85d5d5b79d8a5c9a9550a85c25723354337336272；pair diff 6bface9d305e426cb24983034baa5e62017b3dd38c5810cf66688dd54048f452。原pair目录c2-query-diagnostic-timeout90-v2-preflight-selfpid-attempt2/tools/pair_query_diagnostic_logging.py。精确源库仍OpenPIbc7603c5、bridgee147f600、RMBenchf401f527。

请在Manager独立跑CPU检查与核验部署时并行做紧凑审查：
1. 新pair确实仅两次backend.infer，同一模型对象、恢复同一真实key/同普通输入；除诊断context外不改变调用，不额外warmup。全部ordinary动作/状态/最终RNG严格比较；只把saved actions按冻结backend cast，其余字段不改变。两类passed语义不能混淆或将缺字段/真实差异隐藏。保存off/on普通输出NPZ来自已有调用、上限8MiB、无覆盖旧文件。
2. S-only pipeline实际只能进入serial_lag30两集smoke、recorded验收及一个pair；固定C2GPU6/19460,19462、90/30秒、原H50K30/env100000/100001/query1/2record64MiB，不重跑J/renderer、不开formal/无限retry。核对旧脚本引用和预检fresh leaf/源码保护、失败退出/进程清理的实际路径。
3. 仅报告影响上述真实合同的具体缺陷；正常文件写入之外的竞争/任意恶意fixture不扩大成框架硬化。本次不是科学审稿或生产库改动。证据和源行号足以支撑Manager裁决后立即发布report并结束，不等待GPU。

已有作者12项CPU/static检查在 local-validation；Manager会独立运行，审查者可按需用CPU少量反例，结果写本workspace。仅证据/工具只读核查无需额外源码worktree；若真需修改独立代码fixture请在本workspace保持隔离，不改作者树。
