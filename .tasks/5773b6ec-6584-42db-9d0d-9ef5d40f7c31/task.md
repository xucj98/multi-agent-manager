...
## Manager选定严格同版本正式对照
已确认本机put_back_full_t_plus_1_s0_20k_100ep完整69/100、31正常任务失败、0 runtime_error，episode0–99/seed100000–100099。正式基线批准，C通过阈值64–74/100。原始结果/launch.json/command.txt/final_review.json位于本机RMBench/eval_result/memory_chunk_20260910/put_back_full_t_plus_1_s0_20k_100ep；checkpoint为本机openpi/checkpoints/pi05_rmbench_put_back_block_full_t_plus_1/memory20k_e7e5ac54_put_back_full_t_plus_1_s0/20000。
选择C额外建立严格旧版本评估worktree，运行版本RMBench3e69b1e665a8eac0104d261b233f1b3339007e00、bridge8ea6078543a875b5ae223df16891cdc1fe975c66、OpenPIa869498f01a246752d7e5c6ed5ccd5dfdd9b3ff4；不需先补跑本机最新代码100。clone仓库/一键环境脚本仍按任务准备，运行树固定旧commit。旧bridge的memory依赖是其既定环境一部分，可正规安装对应OpenPI轻量client到旧评估环境，不以临时PYTHONPATH糊旧依赖；用户取消的是最新机器人部署对此包的依赖，不是修改历史运行版本的事实。最新bridge可运行性另按其无客户端依赖验证。
允许开始同步这个checkpoint及put_back必要资产/评估数据，不需要所有训练数据。原运行只有5条video其余no-video，精确继承实际配置，不从report笼统100条video判断有100个MP4。核对hash/命令/seed/HK后开展三机smoke及完整100。
另本机其他run存在episode启动RPC超时公共故障，Manager会独立安排处理；C若同类故障复现，保留证据及时报告，不擅自改超时/忽略错误。严格对照代码不混入未审批的修复。
