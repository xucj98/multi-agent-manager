...
2. **put-back 的 300 集存在强烈的轨迹聚类，不能按 300 个独立样本处理。** Manager 复算显示，每个 `arm × eval × origin_mat` 组在已核对的结果和 episode length 字段上完全相同；这表明重复 seed 行共享明显的 origin/执行模式。它不等于已经证明有效样本量是四，也不证明每组是四条完整物理轨迹。300 集成功率仍可作为 benchmark 描述分母，但不能据此把简单 episode bootstrap 当作独立试验不确定性。
...
4. **origin 分层显示 cohort 方向反转。** put-back 的 fixed 在 eval0 使 front/right 两类全部失败，eval1 四类均成功，eval2 只使 right 失败；event 的变化也集中在少数 origin cells。这个结构提示策略、场景类和运行时可能存在交互，但不能仅凭分层表断言其来源。应报告 `eval × origin × arm` 的 cell 表和 length，而不是只报告 300 集总数。
...
- **event 触发重规划改善：尚不支持稳健因果结论。** put-back pooled `+14.33 pp` 很醒目，但由三个 cohort 的不同 cell 组合产生；rearrange 仅 `+1.33 pp`，event 还低于 fixed。触发数 `20/50/79` 也不解释该差异。
...
1. **先冻结分析口径。** 将 18 批标成“数值已由 Manager 复算；逐 lane 终审状态另引 receipt”，补一张 `arm × eval × context/origin` 表，列出 `n`、success、episode length、trigger、GPU/lane、runtime commit、preflight hash 和 arm 顺序。put-back 的 bootstrap/显著性区间改为不报告，或明确按 origin 分层作描述；不能把重复 seed 当独立轨迹，也不要把有效样本量硬编码为四。

2. **先看干预前首动作差异。** 对同一初始 observation、模型、action RNG 和匹配场景，比较 baseline/fixed/event 在任何 probe、cache 更新或 event trigger 发生前的首个 ordinary action/chunk。首动作已经不同，就说明后续成功差异不能直接归因于刷新或重规划；首动作一致而后续首次更新后才分叉，才有资格继续检查 state/cache 路径。应保留首动作输入、输出哈希、RNG、lane 和时序证据；这是比终态成功率更直接的最小机制诊断，不需要新实验。

3. **做现有证据的只读一致性核查。** 确认 `expert final_block_pose` 是否进入 policy input、success 判定、终态或仅是 preflight 记录；核对三臂的 runtime/compiler/GPU、进程顺序、action/probe RNG 和完整初始状态。若不能证明其无关，保留“full preflight identity 未匹配”的限制。这个核查不需要重跑模型。

4. **HF/LR 优先级降级。** cba8b004 建议的 LR 仍是概念上正确的区分（fixed 的中间递推 versus 边界前刷新），但在当前 fixed/event 的 cohort/lane 异质性未解释前，不应把 LR 或更多 HF 批次列为 P0，也不应追跑到显著。若将来仍把“中间递推”作为主问题，先用 lane/arm 交叉和完整 identity gate 重新设计，再决定是否做 LR。

5. **cover 结果保留为待诊断的描述性记录。** handoff 的 wave1 复算中 `cover_blocks N train0 eval0=0/100`，且失败为 `unspecified_failure`；这不能证明 adapter/schema 故障，也不能证明科学上的零效果。保留全部 episode，继续正常覆盖排程；在使用它作为机制确认前补查失败原因和终态记录，不因该零结果删除重复 episode、阻止其余主线覆盖，或直接启动/否定 HF 三臂。

6. **V 设为工程准入门。** 双卡 OOM 后暂停 9 个 V 训练；先固定完整 18-slot 输入、global batch32、reset/mask/cache 合同和实际显存/吞吐验收。不能逐任务缩图或减 batch 后把 V 混入主表；在 V 无 checkpoint 前，论文只保留“计划比较”，不写显式状态优于视觉历史。

7. **保留低风险主线，收缩推断。** 九任务 N/S/J 覆盖、已有 checkpoint 的三 eval 补齐和 J/T/JE/C 合同审计可继续按资源计划进行，但所有单 train seed 结果均标为 checkpoint-conditional；JC/SF、全面 K/row 网格、额外按钮字段训练继续后置。新增 training seed 不应因为 HF pooled 差异而自动扩展。
