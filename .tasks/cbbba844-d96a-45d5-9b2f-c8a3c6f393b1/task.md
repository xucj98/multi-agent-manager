...
## Manager归档准备裁决
独有材料必须保留，但不意味着必须永久保留整个执行workspace。请将training_logs/、launch/、validation/（排除__pycache__）及三库任务分支的可恢复git bundle复制到稳定路径`/mnt/public/xcj/Projects/openpi/checkpoints/u_training_provenance_cbbba844/`，写manifest记录原路径/commit/每文件SHA256，并git bundle verify。不要复制环境、共享数据或模型；不改已验收checkpoint manifest。核对与正在跑的评测不依赖这些task worktree（有依赖立即报告）。成功后删除本任务这些已经逐文件校验归档的workspace独立附加副本及明确可再生cache，保留三个已登记worktree供Manager执行mam task archive。不要自己删worktree/branch或运行archive；不改任何代码。报告归档准备可执行/阻碍及稳定存档路径。此处允许受控清理已经有逐文件同值存档的任务临时材料，覆盖上节“不删除任何内容”的本轮限制。
