...
Do not collapse automatic user steer/native manager-message wake into manual cancelled; use reason `message`, display `received new message`, and include the affected agent ID. User steer and Manager input share this reason; manual stop remains `cancelled`. Preserve compact actionable error and empty-target reasons already required. No full metadata dump. Add assertions for output reason and ALL required fields, including manual cancellation and timeout. Ensure CLI output (not just internal return dict) retains these fields.

## Final approved state-based pending-work semantics
Supersedes any earlier suggestions of completion cursors, catch-up queues, or dedup acknowledgements. NO delivered-event cursor, unread queue, public ack, or historical replay mechanism.
`mam wait` means caller has handled its work and is ready to wait. It first evaluates CURRENT state. Relevant stopped/unarchived job -> immediately RETURN with reason/IDs/note, never print then keep blocking. For manager, inactive executor with unarchived task -> immediately RETURN agent-id/task-id/task-title, even if there was no event while listening. If owner becomes active, its jobs are its responsibility again; manager may wait on that owner without waiting for job archive. Jobs remain until owner handles/archives them; no auto archive.
Review responsibility: an unarchived review task linked to source task suppresses the SOURCE executor's inactive-task trigger. Manager evaluates/waits on the reviewer instead. An inactive reviewer with unarchived review task triggers immediate return. Once review task archived, source inactive/unarchived task becomes pending for manager again. Review delegation does NOT hide source stopped/unarchived jobs with inactive owner. Handle review links safely without cycles/global unrelated tasks. No new task state required.
Subagents doing long jobs should keep active turn via mam wait; their turn completion hands responsibility back to Manager. Evaluate state before blocking and after events so completion between waits is naturally visible. Subscription/snapshot race still must be closed, but no notification catch-up ledger. Repeated returns are intentional until underlying pending state is resolved/delegated.
User specifically requests Manager use native wait_agent with timeout_ms=300000 AND outer functions.exec yield_time_ms=300000 during implementation; no short polling loops.
Acceptance matrix includes active owner/stopped job exclusion for manager, inactive owner/stopped job return, source under active review, inactive reviewer, archived review exposing source again, and completion before wait by current state.

## Installer contract update
Other task now provides scripts/install.sh with .bashrc setup, explicit terminal yes before app-server restart, then compatibility tests. Coordinate require_compatible with it; no runtime version allowlist. Runtime implementation must not itself kill/restart server.
