...
审查意见裁决（保留原独立report，不要求审查者改成PASS）：
1. 根级passed是“两类检查均通过”的保守合取，不会合并或覆盖两个已明确具名的科学结论；现有receipt分别完整保存logging_invariance和cross_process_saved_replay。原task要求分开报告，没有禁止另外保存整体合取。S pair是流水线最后阶段，退出1不会丢掉已完成smoke或触发重跑。Manager验收以各自字段为准，不把根failed说成logger改变行为，也不把invariance=true说成跨进程重放成功。该意见为汇总表述建议，不阻塞本次运行。
2. ordinary comparator遇首个差异即返回false，不会让后续状态差异变成整体true；false时未遍历字段不能称相等。新工具保留两侧全部ordinary数组及字段身份，后续可离线逐字段量化。首差短路是诊断详尽度限制，不是错误准入；不要求为此修改冻结工具或增加GPU。报告若遇false需说明后续字段尚未比较，必要时用保存数组离线补算。
3. 新简短manifest未重复列validate_timeout90_config.py属实，但该文件已列入之前attempt2部署receipt0a2809fd871679bf425fbfc0d9aa443da0ac54b82ec6da9e1b73d9c1011f581d的verified_files。Manager已在C2重新计算该旧receipt关联28个文件及新增6个文件；实际validator SHA2e84d49ae71d605f411b5b4ab0ec845d729ef857e024e35838abfc4fb8d35f09与本机冻结源一致。这里以旧完整清单+新增工具清单+本次重核作为内容身份链，不声称仅新清单单独完整。启动前继续核对上述validator实际hash并留在launch证据，无需重发/改写旧manifest或重建验证框架。

现明确授权同一terra/max执行者直接运行已部署S-only入口：
`/mnt/public/xcj/Projects/state-vla/workspace/8968b7f5-74f7-44db-ad0b-058d3fd556ca/records/tools/query-diagnostic-c2-timeout90-v2-invariance-replay-s-continuation/run_serial_lag30_continuation.sh`

先按现有预检确认C2GPU6/19460,19462空闲，三库仍bc7603c5/e147f600/f401f527 clean，工具/旧依赖hash与以上链一致、S三个leaf及status/log不存在。固定put-back S serial_lag30 train0/20000、episodes0/1/env100000/100001、有/无视频、query1、H50/K30、原协议/90秒首推理与30秒后续、2record/64MiB。S两个recorded验收成功后，同一实际输入/恢复key只做一次off/on pair（两个backend.infer），ordinary输出8MiB上限。无J重跑、renderer重跑、HF、正式100、机制采样或新训练。

任何基础设施/记录失败停止并保存首因；pair无论哪一结论失败都保留其原始非零退出和两个独立结果，不自动重试或扩大预算。即便invariance通过也不能仅凭该项宣称整个P0完整准入。S收尾后检查真实logits/selected/action-condition、动作/执行证据、数组/hash及最终RNG，并分别报告两种比较和未观测范围。预计>30min登记MAM实际PID，结束turn由MAM唤醒；全部完成后归档job、发布报告，由Manager最终验收。此次授权覆盖之前等待Manager的S暂停，不需要再次询问许可。

## 当前接续：离线明确 pair 的两种比较边界，准备 S 接续

Manager 进一步独立验收 J 记录链：acceptance 收据 SHA35fc6e092c51910b49c522737c044246d76017cb1aaa942b949f681c6992c412及其10个引用文件全部重哈希；对两条episode0/1、env100000/100001、query1实际调用冻结validate_record均为recorded、各38数组。逐数组确认policy after_output_transform经float32转换精确等于记录的post-wire H50，实际execute精确等于其前30×14；两条均accepted且controller progress实测completed30/queued0。结果summary completed、episode_count2，两个scheduler均episode_terminal/returncode0，首次90秒日志标记均存在。接受本次两条J记录及上述执行关联；没有据此接受跨进程重放、S或完整P0准入，工程两集不计入论文正式44批。
