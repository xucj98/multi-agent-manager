# 空白review MAM精简列表与查询行为

## 你的任务：独立review，不执行下附source实现任务

先读MAM AGENTS/README开发验证，以mam task show中的固定source任务和report为准。用mam workspace add --repo multi-agent-manager --base 23288c32a37af56b20e770ae07ff4045b2448f73创建自己的独立环境/worktree；仅在独立临时root上测试CLI，不能改真实任务数据、系统安装或其他任务工作树。

审核新list的一行五列及完整UUID、未绑定/unknown、--json完整性；show默认可读正文及发布revision/历史revision/JSON；status只读已有job结果、没有隐式进程探测；job list静态筛选与实时running/stopped/unknown/attention的正确组合，不查询不需要的归档agent；其他create/publish/status/job/workspace命令保持原JSON。兼顾README/help是否准确解释状态与输出变化。

先按普通使用者执行最小CLI验收，再审代码和已有测试。无需为偏好扩大接口，不增加自动化或依赖。运行标准库单测及diff-check；检查归档/发布等原保护没有被输出层误伤。只有发现真实问题时增加独立复现例，不镜像实现写大批测试。报告GO或按严重度列具体问题/证据/最小修改建议，区分阻塞项与可选建议。

不修改实现、不占GPU、不派agent、不安装系统mam。临时数据和测试产物自行清理，worktree留Manager归档。report包含task_revision、被审commit、自己workspace、验证与结论，并发布。预计20分钟内交付。下附source为审阅对象与验收约束。

Review fixed source delivery:
{
  "task": "b2646623-9cef-4e0f-8695-06baa8b930f7",
  "task_revision": "0013b2aee851a1b2fee75cd151d3bd3482a20435",
  "report_revision": "227e0f74dbfeb9cbbe70358c0ffc13ed790dd4cd",
  "commits": {
    "multi-agent-manager": "23288c32a37af56b20e770ae07ff4045b2448f73"
  }
}
