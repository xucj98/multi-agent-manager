...
本条替代此前持续监测PM、保持active直到服务停止或现场交接的要求。部署交付已完成，常驻PM的生命周期不属于本任务剩余工作。用户指出无变化等待造成大量模型请求；现在停止自己的mam wait并结束turn，不再轮询、记录无变化超时或续写等待报告。

仅将report中的等待责任更新为：部署交付已完成；现场常驻服务仍由Policy Manager管理，当前不承担持续监控。保留真实job状态，不停止/重启服务，不伪报stopped，不自行归档仍运行job。发布一次简短收尾report后直接结束turn。Manager负责处理登记与归档的工具限制，不要因为task/job未归档重新进入等待。无需再次SSH核验、重跑测试或新建环境。
