...
- `execute()` 保留已构造 checker 的 calls、budget、record 和 artifacts；即使末次 key 物化或清理失败，也不会回退成新的 `0 action + 0 probe` precheck receipt。artifact writer 会独立尝试每一个已完成 pickle，并把单个写入失败写入 `serialization_errors`。
