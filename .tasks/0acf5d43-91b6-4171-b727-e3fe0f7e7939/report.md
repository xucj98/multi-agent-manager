...
终态 receipt 为 `/mnt/public/xcj/Projects/state-vla/workspace/0acf5d43-91b6-4171-b727-e3fe0f7e7939/formal_first_layer_j_20260914/receipts/formal_hf_fixed_put_back_evalseed0_terminal_audit_20260915.json`，SHA-256 `0f13304c0a453867fedf6549386d48c3da643a5b04d419f3e96c3d2960081b54`。审计工具的 eval0 接线 state commit 是 `96a66355e80a811023ee9095d0d730b0d8dec306`，receipt state commit 是 `927a84b408bc0480e972a1b4892fc00f7bcb69b7`。审计核对 GPU0/`19400,19402`、冻结 clean 三库 HEAD（OpenPI `0ce566bd34f99cb4775422f012ab67c16aa53885`、robot-bridge `ffa122494c19e1c0154e877010f7b470967ccfc6`、RMBench `6abebf08d084d0be43aa56ebe158dc8395fa58e4`）、checkpoint/metadata，以及冻结 manifest `469f2b270bbea57da6d6a06fd5787f20262e07153dfe3be89fe8657542cb3d26` 与 scheduler `1b411a2a4adf0bcf2042dbbdf021b12d4a6a0e000b48df733c48895d16bee8f9`。
...
MAM stopped job `3f95aee4-08aa-4c18-a763-915e3cccc0db` 将在本终态报告发布后归档。按已授权固定 lane 顺序，随后只启动 GPU0 的 put-back HF-event eval0 strict matching-smoke2→formal100；不改变任何 seed、阈值、runtime、checkpoint、source 或冻结输入。

# 置顶：第一层 J put-back HF-fixed eval1 终态审计（2026-09-15）

C3 GPU1 的 `c_hf_j_hf_fixed_put_back_trainseed0_evalseed1_100ep` 已停止，但停止并未直接被视为成功。task-private `audit_formal_hf_fixed_put_back.py` 先完成一次不写入 dry audit，再实际调用 recorder 的 `validate_smoke_run()` 和每集 `check_rolling_evidence(..., required=True)`；两次检查均通过，结构化 BenchmarkRunner summary 为 `completed`、`error=null`、target `100`。因此该批被归类为正常完成：连续环境 seed `200000..200099` 的 100 个 preflight 全部 accepted，100 个 episode 都是正常终态，结果为 **100/100 Success**、0 个 ordinary failure。

正式终态 receipt 是 `/mnt/public/xcj/Projects/state-vla/workspace/0acf5d43-91b6-4171-b727-e3fe0f7e7939/formal_first_layer_j_20260914/receipts/formal_hf_fixed_put_back_evalseed1_terminal_audit_20260915.json`，SHA-256 `4e45b92db5d34fa4bf871fa1c453f93518816bf44e2bb9a5edc30ae1a8edaa8e`。其审计工具 state commit 为 `f2013d9bb4b5955a69eb3c59598a0ae48b1eaec5`，receipt state commit 为 `02558cf4a993e1c3457354d605907d7ec5cbd2ae`。审计核对 GPU1/`19410,19412`、冻结 runtime 三库 clean HEAD（OpenPI `0ce566bd34f99cb4775422f012ab67c16aa53885`、robot-bridge `ffa122494c19e1c0154e877010f7b470967ccfc6`、RMBench `6abebf08d084d0be43aa56ebe158dc8395fa58e4`）、checkpoint/metadata、manifest `deebe19d37aff927ab1c04a275877459eae526699eff3aa4124da1be3d8a97ed`、scheduler `1b411a2a4adf0bcf2042dbbdf021b12d4a6a0e000b48df733c48895d16bee8f9` 与新的 strict matching-smoke2 config `1901c4ff8373c9486821ba28f1cf26a9c525d86f7c7cf7e473813fd54243be23`。域随机化仍为 `random_light=false`、`crazy_random_light_rate=0`。
...
GPU1 的两个端口已释放、GPU1 为 `2 MiB` / `0%`，stopped MAM job `0283a094-b073-4a95-ba7c-be67cadbd924` 已在终态报告发布后归档。随后 MAM snapshot 显示 GPU0 eval0 的 HF-fixed job 已停止、GPU2 eval2 的 HF-fixed job 仍在运行；这里不把 GPU0 的停止或 GPU2 的运行状态当作结果，它们分别仍需独立收尾或等待终态。GPU1 已按固定 lane 顺序启动 put-back HF-event `c_hf_j_hf_event_put_back_trainseed0_evalseed1_100ep`：outer PID `3187488` 在启动三秒及 MAM 登记时均存活，MAM job 为 `88a440d4-60b1-464a-a492-84cdefecee8d`。该 outer 正在运行新的 strict matching smoke2，只有 identity/evidence/quiescence gate 通过才会 `exec` 原 formal100；seed、阈值、runtime、checkpoint 和冻结输入均未改变。
