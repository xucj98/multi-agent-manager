# 新增两批put-back正式评测独立证据核验：J/T train1 eval1
...
独立重哈希 final review 引用的证据，重读 diagnostics 的真实 diagnostics.episode_status 与 preflight：100条、episode0..99、固定200000..200099无跳过无拼接、terminal和success及故障分类。核实 matching smoke 留存 review/cleanup 身份、命令/config checkpoint训练seed1/eval1、H50/K30、90/30秒、原 RNG 生命周期、三库冻结 f401f5279c95451eb424ac98b831bab5552b2120 / f9626636c4776d8eb15f9c556775cb2d12c000e5 / a869498f01a246752d7e5c6ed5ccd5dfdd9b3ff4。检查100集视频策略记录、scheduler/process退出与日志错误；可依赖 owner 实际视频解码收据但明确未重复解码部分。不要新启动任何模型/仿真/训练/复制大资产，不触碰仍运行的 eval2，也不归档源task或更改源报告。

仅以本 task 目录保存小型只读审计 receipt（含原件路径/hash、独立算术、未核实的限制）并发布 report。无源码修改，无需创建无关源码 worktree。最终提供是否有阻止将这两批计入论文的证据缺口；Manager 将独立抽核后裁决，不由你直接更新论文或总数。完成后结束turn。
