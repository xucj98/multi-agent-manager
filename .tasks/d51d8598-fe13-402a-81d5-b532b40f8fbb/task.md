...
# Manager accepted shell-ownership blocker

Your BLOCK on unmarked .bashrc exports is accepted and assigned to installer owner with a narrow preservation/idempotency/syntax regression. Prior source/bootstrap/wait/coexistence/docs and187-suite review remains PASS; only recheck the forthcoming shell-ownership fix and final combined tests needed to close this review, do not restart broad audit or add real model tests. Manager will notify exact fixed commit. Report can remain published BLOCK until that candidate is ready.
