...
用户明确指出：只有一个场景和一条轨迹经其本人验收，之后的大多数结果不满意，项目允许从头做。
这条口径优先于 roadmap、worklog、测试状态和历史“完成”措辞；除下文标出的候选外，本报告不把
任何历史场景、计划、视频或工作包当作已获用户验收，也不建议继续沿原路线扩展。
...
旧仓库可作为参考实现，其中包含 ManiSkill backend、schema/validator、Open/Explicit 视图隔离、
集合值 outcome evaluator、标注 server、reference/native replay 和小型物理 oracle。这些只是可审计的
工程现状，不代表用户认可或新项目的技术路线约束。在本次可审计的提交数据与工作记录中，尚未发现
Phase-0 所需的真实人类标注、human-calibrated evaluator、真实 VLM/VLA 结果或可声明为 SOP-open 的
release；新项目若选择复用其中任一部分，仍应按用户验收口径重新验证。
...
| 人类标注与偏好模型 | 两阶段 annotation server、freeze、coverage audit 和去标识导出已实现；协议明确 10 人独立布局、冻结后 pairwise 的目标和 coverage 门（`docs/spec/human-pilot.md:16-109`；`src/table1000/annotation/study.py:81-184`）。 | 本次可审计的提交数据与工作记录中未发现 annotation SQLite、export、coverage 或真实参与者记录；不能据此断言所有未提交历史产物都不存在。proposal 要求的每场景多位独立人类终态、anchor 密集 pairwise、human ceiling 在已审计记录中尚无证据（proposal `docs/proposal/research-proposal.md:324-370,402-430`）。 |
| Baseline/模型实验 | 三个规则 baseline 有一次真实 `physx_cpu` 运行：5 个 probe × 3=15 episodes；DoNothing/Random/Category 的 required relation 都为 0/42，后两者均首个技能失败（`docs/worklog/baseline-first-run-2026-08-15.md:3-19,63-88`；commit `7eed18e`）。VLM runner 可接 stub/replay/configured live provider（`src/table1000/baselines/vlm.py:119-162,256-358`）。 | VLM 首轮是 offline deterministic stub，记录明确说明不是任何真实模型能力分数；5/5 skill failure、0/42（`docs/worklog/baseline-vlm-first-run-2026-08-15.md:5-20`；commit `489750e`）。本审计范围内未发现 live VLM/VLA checkpoint、模型版本、真实 provider response 或 Open track 主结果；这不对未提交历史产物作绝对断言。 |
| 长程执行/轨迹 | Probe-0001 有两组配置不同的真实物理部分证据：早期 fresh-build projected-goal 记录为 5/14、13/13 已发技能通过（`docs/worklog/probe-0001-physical-tidy-2026-08-20.md:14-36,89-129`；commit `ea73166`）；后续自由位姿收纳复核在显式 run-level drift relaxation 下几何进入 9/14、按主支撑合同为 7/14（`docs/roadmap.md:239-247`）。 | 5/14 与 7/14 不是同一设置，不能合并成全局 ceiling；两者都不是完整 tidy trajectory，也不能作为 IL demonstration。低层 grasp/placement/packing 仍会混淆 Track-1 的高层规划归因。 |
...
4. **未完成：Phase-0 的多解性与可判定性。** proposal 的 go/no-go 需要独立人类终态与高于随机的一致性（`docs/proposal/research-proposal.md:669-677`）；本次审计的提交数据/记录没有满足它的证据。
5. **未完成：真实人类 preference model / calibrated evaluator。** 现有 fixture evaluator 和 author priors 不能替代 pairwise、human ceiling、校准或 population/persona 结论；本次审计范围内未见相反证据。
...
7. **未完成：论文级模型比较。** 本次审计范围内只有失败诊断的规则 baseline 和 VLM stub；缺 Open–Explicit 成对、真实模型、执行归因、置信区间及 leakage/generalization split 审计。
...
2. **执行归因瓶颈：semantic outcome、可放置终态与可执行长程轨迹尚未连接。** Table-10 的 oracle QA 是积极信号；但 dense probe 的早期 5/14 与后续、放宽 run-level drift 后的 7/14 都不完整，Open 的三-seed 物理验证记录又为零。现阶段不能把模型失败归因给 goal induction/规划。
...
## 如重新立项时可用的边界清单（不构成旧路线续作）

| 工作包 | 交付物 | 验收标准 | 依赖与边界 |
|---|---|---|---|
| A. 场景静态终态 QA | 一套可冻结的 pilot scene manifest；每 scene 的资产/provenance、初态、≥3 语义不同 outcome、负样本、outcome signatures、render review 与静态/settled QA report。先替换或退役 0006，并新增“逐对象终态差异/容器视觉语义”硬门。 | 自动检查拒绝零状态差的不同 cluster；每 accepted outcome 在 disposition/assignment/关键关系之一不同；资产容器的 visual/collision/provenance 审计通过；所有静态报告可从 manifest 重算。 | 可立即并行于 B/C；不以完整轨迹或三-seed execution 为人评前置。不要把 author fixture 计作 human support。 |
| B. 独立人类多解与 evaluator 校准 | 小规模、版本化两阶段 pilot：同意/招募记录留在私有区，匿名 layout export、冻结 pairwise 图、coverage/human-ceiling/一致性/顺序偏差报告，以及冻结的 evaluator calibration split。 | 每场景独立布局和 pairwise coverage 达到 `docs/spec/table10-open-v1.md:94-111` 的门；原始 human、author fixture、near-miss 分层保存；报告明确样本量和不确定性。 | 实质采集只依赖 A 选出无伪多解且可审阅的 scene；需项目负责人解决招募/伦理/数据管理。可先并行完成 dry-run、界面可用性和导出审计。 |
...
| 公共 P 线：完整轨迹 release gate（资深维护） | 对冻结的 accepted outcome/strategy family 产生 0/13/37 backend-native 完整轨迹、event trace、post-settle canonical state 与 failure taxonomy；独立保存训练可用 trajectory manifest。 | 每条进入 Open 物理/IL release 的轨迹通过 reset、skill、hard constraints、稳定性和功能检查；区分 run-level drift relaxation、支撑合同与严格 release 门。 | 依赖 A 的静态 package 与 solver/oracle；它是 Open 物理 release 和 IL 数据的门，不阻塞 B 的人类偏好采集。 |

若新项目选择复用旧 solver，dense solver、grasp/placement 与完整 trajectory recovery 可作为公共 P 线；已记录的 5/14 与 7/14 均是不同设置下的部分结果，不应视为已解决低层执行，也不应成为新路线的默认前提。
...
## 视觉复现交接（只给候选路径与命令，未执行）

- **唯一有明确“负责人验收”文字证据的场景/终态候选：**
  `data/probe/scenes/t1k-probe-office-dense-0001-v1/scene.yaml`（初态）和
  `data/probe/scenes/t1k-probe-office-dense-0001-v1/scene-tidy.yaml`（`clear-for-tomorrow` tidy）。
  `docs/worklog/scene-metrics-layer2-2026-08-15.md:28,64` 与
  `docs/worklog/baseline-first-run-2026-08-15.md:114-115` 均称只有 0001 tidy 是负责人验收基准。
- **轨迹候选需由用户确认：** source plan 是
  `data/probe/scenes/t1k-probe-office-dense-0001-v1/plans/reference-plans.yaml` 的 `tidy-video`；
  当前文档把它作为 oracle plan，不能仅凭此视为已验收的完整轨迹。历史物理视频最可能的候选位置为
  `outputs/solver-runs/probe-0001-physical-tidy-best-effort-2026-08-20/probe-0001-physical-tidy-best-effort.mp4`
  （worklog 记录 SHA-256 `6f8ad6088032e74ef17260e9b8177217151d1e29d946be855538505ee2b8d3cc`）；另有
  `outputs/solver-runs/probe-0001-verified-5-of-14-not-full-2026-08-15/` 和
  `outputs/solver-runs/probe-0001-terminal-target-attempt-all-2026-08-16/terminal-target-best-effort.mp4`。
  它们均为 gitignored 历史位置，本次 workspace 未发现文件，且名字/工作记录本身不等于用户验收。
- **稳定场景抽查入口：**
  `python scripts/probe_scene/render_strip.py --package data/probe/scenes/t1k-probe-office-dense-0001-v1 --camera cam-top --output-root outputs/review-0001`
  会从磁盘同时识别 legacy 0001 的 `scene-tidy.yaml`；见 `docs/guide/review-renders.md:14-39`。
- **0001 前后终态对比入口：**
  `python scripts/probe_scene/render_compare.py --legacy-goal-config scripts/probe_scene/configs/legacy-office-dense-0001-goals.yaml --output-root outputs/review-0001-compare --hide-zone-markers`。
- **计划执行的静帧/视频诊断入口：**
  `python scripts/render_plan_states.py --scene data/probe/scenes/t1k-probe-office-dense-0001-v1/scene.yaml --plan-id tidy-video --seed 0 --output-dir outputs/review-0001-plan`；
  `python scripts/record_reference_plans.py --scene data/probe/scenes/t1k-probe-office-dense-0001-v1/scene.yaml --plan tidy-video --seed 0 --output-dir outputs/review-0001-video --report outputs/review-0001-video/report.json`。
  这些是 diagnostic render，不应把输出自动视为验收或 release 证据。
