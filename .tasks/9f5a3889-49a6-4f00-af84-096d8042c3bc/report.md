...
- 已接入新发布的跨库 `memory_input_ids` / `memory_prediction_ids` wire 契约，独立提交 `f84edbd`。有 `memory_config` 时，三类 scheduler 发送原始 robot state、images、prompt 与有序 `(F,)` semantic IDs；只执行机器人宽度的 `actions`，并从严格检查的 full `(H,F)` 或 serial `(1,F)` IDs 更新 `MemoryContext`。不再写 dense state offsets、从 actions 尾部解码，或在 serial 路径重新 argmax。`memory=[]` 不要求任一 memory wire field；旧 F0/legacy 分支不改动。
- 该提交的 runtime/context CPU 覆盖单字段、多字段、full K30 row20/row30、serial 实际 selected IDs、no-memory、输出 layout/类别范围和三 scheduler 生命周期。真实 OpenPI input/output transforms → MemoryContext 的跨库 smoke 等训练 owner 的正式 commit；当前没有引用其临时 workspace 或宣称该边界已验收。
...
- robot-bridge worktree: `/mnt/public/xcj/Projects/workspace/9f5a3889-49a6-4f00-af84-096d8042c3bc/robot-bridge`
  ...
  - `f84edbd fix: align memory runtime wire with policy transforms`（新 schema wire；不改 legacy F0）
...
- Memory v1 新 wire 定向 context/scheduler 集合：16 passed，1.59s；完整 `tests/scheduler`：88 passed，27.84s。覆盖 full/serial/no-memory 和 output ID 校验；跨真实 OpenPI transforms 的 CPU smoke 待其正式交付 commit 后补做。
