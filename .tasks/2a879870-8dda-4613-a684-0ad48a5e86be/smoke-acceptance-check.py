...
checks={'exit0':(d/'exit').read_text().strip()=='0','exact2_fixed_seeds':len(ep)==2 and [(x['episode_id'],x['seed']) for x in ep]==[(0,100000),(1,100001)],'accepted2':len(pre)==2 and all(x['accepted'] is True for x in pre) and [x['seed'] for x in pre]==[100000,100001],'terminal_without_error':all(x.get('error') in (None,'') and not x['diagnostics'].get('runtime_error') and x['diagnostics']['episode_status']['terminal'] for x in ep),'video_checks':len(vid)==2 and [(x['episode_id'],x['enabled'],x['ok']) for x in vid]==[(0,True,True),(1,False,True)],'no_video_file':not (r/'episode1.mp4').exists(),'artifact_files':all((r/name).is_file() for name in ['command.txt','config.yaml','scheduler.yaml','episode0.json','episode1.json','diagnostics_summary.json','_result.txt'])}
