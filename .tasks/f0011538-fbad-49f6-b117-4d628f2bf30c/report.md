...
- 已实际读取 `/root/Documents/task-state-vla-paper/docs/EXPERIMENT_PLAN.zh-CN.md` 与 `ASSET_AUDIT.zh-CN.md`。后者确认这两个任务均无 `demo_clean_state` source 或 converted cache；此前仅搜索 `/mnt/public/xcj` 而声称资料缺失是错误，现已更正。`/root/.cache` 已确认链接至 `/mnt/public/xcj/cache`，且当前 `HF_LEROBOT_HOME` 未设置。
