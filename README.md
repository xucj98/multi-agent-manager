...
所有 agent、代码修改和管理命令都在本机运行。训练、评估等 GPU 作业可通过 SSH 在 `wuwen-1` 运行；管理工具在本机查询两端进程，`wuwen-1` 无需安装管理工具。

本机和 `wuwen-1` 共享 `/mnt/public` 下的代码、数据、解释器及缓存。模型和仿真环境只需创建一次，两端可使用同一个 worktree 的 `.venv`；登记进程时须指定实际运行主机。
