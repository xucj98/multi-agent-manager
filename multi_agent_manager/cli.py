...
def parser():
    ...
    task = command(cli.add_subparsers(required=True), "task", "manage tasks, publications, workspaces and jobs")
    ...
    w = command(sub, "workspace", "manage repository worktrees and their environments").add_subparsers(required=True)
    ...
    jobs = command(sub, "job", "register, query and archive process records").add_subparsers(required=True)
