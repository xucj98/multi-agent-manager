...
- 新 clone 或新 `PROJECT_ROOT` 首次使用时，若 `.local/README.md` 不存在，先按[安装说明](docs/install.md#本地入口文件)补齐本集群的最小入口文件，再开始 task；不要复制旧的 service、task 或其他 runtime 状态。
...
- 开发 MAM 请先阅读 [MAM 开发指南](docs/development.md) 和 [MAM 设计细节](docs/task-management-design.zh-CN.md)。修改 MAM 文档，如 `README.md`，`AGENTS.md`，`docs/` 也属于开发MAM。
