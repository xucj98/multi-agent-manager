...
- Manager 先创建任务、编辑并发布要求，再启动执行者并绑定 agent ID。每个执行者对应一个未归档任务及其 UUID workspace。
- 执行者先用 `show` 读取发布要求和版本，再阅读涉及库的 `AGENTS.md` 及任务所需规范。追加要求由 Manager 写入任务、发布后通知执行者。
- 修改代码和独立代码 review 通过 `workspace add` 创建各库 worktree，复用本任务已有工作区。讨论、只读调查和监控登记任务即可，无需创建代码环境。
...
- 执行者登记长任务进程，按任务要求收尾后归档 job；Manager 查看待处理进程并通知已空闲的负责人继续工作。
