...
- 接到任务后，用 prompt 中的 TASK-ID 按[执行与交付](README.md#执行与交付)查询工作区和发布要求，再阅读任务涉及库的 AGENTS.md。
...
- 启动长任务且需要保持 active turn 时，使用 `mam wait jobs [--task TASK-ID] [--timeout TIMEOUT]` 等待。
